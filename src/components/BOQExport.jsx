...
const BOQExport = ({ isOpen, onClose, boqItems }) => {
  ...
  const exportToExcel = () => {
    const worksheetData = [
      ...
      [''],
      ['Line #', 'Manufacturer', 'Part Number', 'Description', 'Service Duration (Months)', 'Est. Lead Time (Days)', 'Unit List Price', 'Pricing Term', 'Qty', 'Unit Net Price', 'Disc (%)', 'Extended Net Price'],
      ...boqItems.map((item, index) => [
        index + 1,
        item.manufacturer || 'N/A',
        item.partNumber || 'N/A',
        item.isDependency ? `└─ ${item.name} (Required by: ${item.requiredByName})` : item.name,
        ...
        item.discount || 0,
        item.quantity * (item.unitNetPrice || item.unitPrice)
      ]),
      [''],
      ['', '', '', '', '', '', '', '', '', '', 'Total Extended Net Price:', totalValue],
      [''],
      ['Notes:', projectNotes]
    ];
